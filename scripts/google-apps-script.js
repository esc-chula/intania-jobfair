        ...
        if (typeof value === "string") {
          ...
          if (looksLikeJson) {
            try {
              ...
              Logger.log(
                `Failed to parse ${header}: ${e.message} - Value: ${trimmed}`,
              );
              obj[header] = value;
            }
          } else {
...
function updateGist(files) {
  ...
  if (!config.GIST_ID) {
    throw new Error(
      "Missing or invalid GIST_URL in Script Properties. Please provide a valid Gist URL.",
    );
  }
  ...
  if (statusCode !== 200) {
    throw new Error(
      `Failed to update Gist: ${statusCode} ${response.getContentText()}`,
    );
  }
...
function updateAllSheets() {
  ...
  updateGist(files);
  Logger.log(
    `Updated ${Object.keys(files).length} files in Gist: ${Object.keys(files).join(", ")}`,
  );
}
