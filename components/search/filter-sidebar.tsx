...
export const FilterSidebar = ({ filters, updateFilter, uniqueMajors }: FilterSidebarProps) => {
  ...
  const studentYears = [
    "Bachelor's Freshmen", "Bachelor's Sophmore", "Bachelor's Junior", "Bachelor's Senior",
    "Master's", "Doctorate"
  ];
         ...
         <FilterSection title="Majors">
            <div className="max-h-60 overflow-y-auto pr-1 space-y-1 scrollbar-thin scrollbar-thumb-gray-200">
              {uniqueMajors.map(major => (
                <label key={major} className="flex items-start gap-2 cursor-pointer group hover:bg-gray-50 p-1 rounded">
                  <input 
                    type="checkbox"
                  ...
                  />
                  <span className="text-xs text-gray-600 group-hover:text-gray-900 leading-tight">{major}</span>
                </label>
              ))}
            </div>
         </FilterSection>
