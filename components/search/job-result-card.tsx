          ...
          <div className="h-12 w-12 flex-shrink-0 overflow-hidden rounded-lg border border-gray-100 bg-white relative">
             {company?.companyLogo ? (
               ...
               />
                ...
                <div className="flex h-full w-full items-center justify-center bg-gray-50 text-xs text-gray-400">N/A</div>
             )}
          </div>
          <div>
            ...
            <p className="text-sm font-semibold text-gray-600">{company?.companyName_en || company?.companyName_th}</p>
          </div>
        </div>
        ...
        <div className="flex flex-col items-end gap-1">
          ...
          {(booth1 || booth2) && (
             <div className="flex gap-1 text-[10px] text-gray-400 font-mono mt-1">
                {booth1 && <span title="Day 1 Booth">D1: <strong className="text-gray-700">{booth1}</strong></span>}
                {booth2 && <span title="Day 2 Booth">D2: <strong className="text-gray-700">{booth2}</strong></span>}
             </div>
          )}
        </div>
      </div>
      ...
      <div className="mt-2 flex flex-wrap gap-2 border-t border-gray-50 pt-3">
         ...
         {job._searchable_majors.length > 0 && (
             <span className="text-xs text-gray-500 bg-gray-50 px-2 py-1 rounded truncate max-w-[150px]">
               {job._searchable_majors[0]} {job._searchable_majors.length > 1 && `+${job._searchable_majors.length - 1}`}
             </span>
         )}
      </div>
    </div>
