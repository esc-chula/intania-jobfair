...
export default function Footer() {
  ...
  return (
    <footer className="w-full bg-primary-yellow">
      <div
        className={`mx-auto h-[52px] flex items-center justify-between px-4 py-2 ${
          isClient
      ...
      >
        <div
          className={`flex items-center ${
            isClient ? "gap-2 sm:gap-3 md:gap-4" : "gap-2"
          }`}
        >
          <Image
            src={cdcLogo}
            ...
            width={144}
            height={36}
            className={`w-auto ${isClient ? "h-7 sm:h-8 md:h-9" : "h-7"}`}
            priority
          />
        </div>
