import { flattenJobs } from "./transformer";
...
if (productionJob) {
  console.log(
    `✅ PASSED: Search by Job Title "Production" found "${productionJob.item.jobTitle}"`,
  );
} else {
...
if (res2.length > 0) {
  console.log(
    `Top result: ${JSON.stringify(res2[0].item.company?.companyName_en)} (Score: ${res2[0].score})`,
  );
}

const valeoJob = res2.find((r) =>
  r.item._searchable_company_name_en.toLowerCase().includes("valeo"),
);
if (valeoJob) {
  console.log(
    `✅ PASSED: Search by Company "Valeo" found job at "${valeoJob.item._searchable_company_name_en}"`,
  );
} else {
...
const companyWithBooth = companies.find(
  (c) => c.boothDay1 && c.boothDay1 !== "-" && c.boothDay1.length < 10,
);
if (companyWithBooth) {
  const boothCode = companyWithBooth.boothDay1;
  console.log(
    `Testing Booth Search for code: "${boothCode}" (Company: ${companyWithBooth.companyName_en})`,
  );
  ...
  const found = res3.find(
    (r) => r.item.company?.companyName_en === companyWithBooth.companyName_en,
  );

  if (found) {
    console.log(
      `✅ PASSED: Search by Booth "${searchTerm}" found company "${found.item.company?.companyName_en}"`,
    );
  } else {
    console.error(
      `❌ FAILED: Search by Booth "${searchTerm}" did not find expected company`,
    );
  }
} else {
