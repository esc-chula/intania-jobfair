...
async function fetchJson<T>(path: string, revalidate = 3600): Promise<T> {
  if (!GIST_RAW_BASE) {
    console.warn(
      `NEXT_PUBLIC_GIST_RAW_BASE not set, returning empty data for ${path}`,
    );
...
export async function fetchJobs(): Promise<Job[]> {
  ...
  const GIST_JOBS_URL =
    "https://gist.githubusercontent.com/tech-esc/411e78247ce49f590a2755d2a7e982fe/raw/jobs.json";
  try {
